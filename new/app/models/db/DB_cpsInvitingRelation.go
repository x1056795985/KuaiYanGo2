...
// cps关系表
type DB_CpsInvitingRelation struct {
	...
	InviterId    int   `json:"inviterId" gorm:"column:inviterId;index;comment:邀请用户ID"` //邀请者一定是账号id   邀请者是余额结算,无需AppId
	InviteeAppId int   `json:"inviteeAppId" gorm:"column:inviteeAppId;index:idx_appid_userid;comment:被邀请AppId"`
	//不同应用可能是不同的邀请者,要区分开来, 用户在该应用充值时,给对应的邀请者
	//还有种情况,账号模式应用 a 第一次邀请的c注册应用1,这时候,b就无法邀请注册了,但是又确实是b邀请的注册应用2,只能让C登陆后,手动填写邀请人
	InviteeId int    `json:"inviteeId" gorm:"column:inviteeId;index:idx_appid_userid;comment:被邀请者用户ID"` //被邀请者可能是账号,可能是卡号,要根据appid分别处理
...
func (DB_CpsInvitingRelation) TableName() string {
	return "db_cps_inviting_relation"
}
