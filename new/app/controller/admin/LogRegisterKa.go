...
// LogRegisterKa 制卡日志
type LogRegisterKa struct {
	Common.Common
}

func NewLogRegisterKaController() *LogRegisterKa {
	return &LogRegisterKa{}
}

// Info 获取详情
func (C *LogRegisterKa) Info(c *gin.Context) {
	var 请求 request.Id
	if !C.ToJSON(c, &请求) {
		return
	}
...
// GetList 制卡日志列表
func (C *LogRegisterKa) GetList(c *gin.Context) {
	var 请求 request.List
	if !C.ToJSON(c, &请求) {
		return
	}
...
type 请求_LogRegisterKaBatchDelete struct {
	Id       []int  `json:"id"`
	Type     int    `json:"type"`
...
// Delete 批量删除
func (C *LogRegisterKa) Delete(c *gin.Context) {
	var 请求 请求_LogRegisterKaBatchDelete
	if !C.ToJSON(c, &请求) {
		return
	}
