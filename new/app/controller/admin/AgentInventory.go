...
type AgentInventoryCtrl struct {
	Common.Common
}

func NewAgentInventoryController() *AgentInventoryCtrl {
	return &AgentInventoryCtrl{}
}
...
type 请求_AgentInventoryGetList struct {
	Page     int    `json:"page"`
	Size     int    `json:"size"`
...
type 请求_AgentInventoryWithdraw struct {
	Id   int    `json:"id"`
	Num  int    `json:"num"`
	Note string `json:"note"`
}

type 请求_AgentInventoryGetTree struct {
	Id int `json:"id"`
}
...
// Info 获取库存卡包详情
func (C *AgentInventoryCtrl) Info(c *gin.Context) {
	var 请求 请求_AgentInventoryGetInfo
	if !C.ToJSON(c, &请求) {
		return
	}
...
// GetList 获取库存卡包列表
func (C *AgentInventoryCtrl) GetList(c *gin.Context) {
	var 请求 请求_AgentInventoryGetList
	if !C.ToJSON(c, &请求) {
		return
	}
	...
	局_DB.Count(&总数)
	_ = 局_DB2.Table("db_Agent_Inventory ai").
	...
	response.OkWithDetailed(响应_AgentInventoryGetList{数组_库存卡包, 总数}, "获取成功", c)
}
...
// Delete 批量删除库存
func (C *AgentInventoryCtrl) Delete(c *gin.Context) {
	var 请求 请求_AgentInventoryDelete
	if !C.ToJSON(c, &请求) {
		return
	}
	if len(请求.Id) == 0 {
...
// Withdraw 库存撤回
func (C *AgentInventoryCtrl) Withdraw(c *gin.Context) {
	var 请求 请求_AgentInventoryWithdraw
	if !C.ToJSON(c, &请求) {
		return
	}
...
// GetAgentTreeAndKaClassTree 获取下级代理列表和可创建库存包列表
func (C *AgentInventoryCtrl) GetAgentTreeAndKaClassTree(c *gin.Context) {
	var 请求 请求_AgentInventoryGetTree
	if !C.ToJSON(c, &请求) {
		return
	}
	if 请求.Id == 0 {
...
func 转换为代理树(代理列表 []*Node, 上级ID int) []*Node {
	if 代理列表 == nil {
		return []*Node{}
	}
