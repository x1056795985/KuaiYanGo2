...
func NewAgentMenuController() *AgentMenu {
	return &AgentMenu{}
}
...
// NewPassword 修改代理Token密码
func (A *AgentMenu) NewPassword(c *gin.Context) {
	var 请求 结构请求_单str
...
// Q取支付通道状态2 取支付通道基本信息
func (A *AgentMenu) Q取支付通道状态2(c *gin.Context) {
...
// Y余额充值 余额充值
func (A *AgentMenu) Y余额充值(c *gin.Context) {
	var 请求 结构请求_余额充值
	err := c.ShouldBindJSON(&请求)
	if err != nil {
		response.FailWithMessage("提交参数错误:"+err.Error(), c)
...
// Q取余额充值订单状态 取余额充值订单状态
func (A *AgentMenu) Q取余额充值订单状态(c *gin.Context) {
	var 请求 结构请求_余额充值
	err := c.ShouldBindJSON(&请求)
	if err != nil || 请求.D订单ID == "" {
		response.FailWithMessage("提交参数错误:"+err.Error(), c)
