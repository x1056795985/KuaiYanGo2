...
func init() {
	rmbPay.L_rmbPay.Z注册接口(pay_支付宝PC)
}
...
func (j 支付宝PC) Q取通道名称() string {
	return "支付宝PC"
}

// 当无法通过订单号,获取订单信息时将循环每个接口,尝试获取订单号
func (j 支付宝PC) Q取订单id(c *gin.Context, 参数 *m.PayParams) string {
	return ""
}
func (j 支付宝PC) D订单创建(c *gin.Context, 参数 *m.PayParams) (response m.Request, err error) {
	var 局_支付配置 m.Z在线支付_支付宝PC
	err = json.Unmarshal(参数.Z支付配置, &局_支付配置)

	if err != nil || !局_支付配置.Z支付宝开关 {
		err = errors.New(局_支付配置.Z支付宝显示名称 + "支付方式已关闭")
		return
	}

	if 局_支付配置.Z支付宝商户私钥 == "" || 局_支付配置.Z支付宝商户ID == "" || 局_支付配置.Z支付宝公钥 == "" {
		err = errors.New(局_支付配置.Z支付宝显示名称 + "服务端未配置参数")
		return
	}

	if 参数.ProcessingType == constant.D订单类型_余额充值 { //余额充值 判断单次最大金额
		if 参数.Rmb > float64(局_支付配置.Z支付宝单次最大金额) {
			err = errors.New("支付金额必须小于" + strconv.Itoa(局_支付配置.Z支付宝单次最大金额))
			return
		}
	}
	...
	if err != nil {
		err = errors.New(局_支付配置.Z支付宝显示名称 + "支付商户私钥载入失败:" + err.Error())
		return
	}

	err = client.LoadAliPayPublicKey(局_支付配置.Z支付宝公钥) // 加载支付宝公钥证书
	if err != nil {
		err = errors.New(局_支付配置.Z支付宝显示名称 + "公钥载入失败:" + err.Error())
		return
	}

	var p = alipay.TradePagePay{}
	p.NotifyURL = 参数.Y异步回调地址
	p.ReturnURL = rmbPay.L_rmbPay.Z支付订单回调关键字转换(局_支付配置.Z支付宝同步回调url, 参数)
	p.Subject = 参数.S商品名称
	p.OutTradeNo = 参数.PayOrder
	p.TotalAmount = fmt.Sprintf("%.2f", 参数.Rmb)
	p.ProductCode = "FAST_INSTANT_TRADE_PAY"

	rsp, err := client.TradePagePay(p)
	if err != nil {
...
func (j 支付宝PC) D订单支付回调(c *gin.Context, 参数 *m.PayParams) (响应信息 string, 响应代码 int, err error) {
	...
	client, err := alipay.New(局_支付配置.Z支付宝商户ID, privateKey, true)
	if err != nil {
		err = errors.Join(err, errors.New(局_支付配置.Z支付宝显示名称+"支付商户私钥载入失败"))
		return
	}
	err = client.LoadAliPayPublicKey(局_支付配置.Z支付宝公钥) // 加载支付宝公钥证书
	if err != nil {
		err = errors.Join(err, errors.New(局_支付配置.Z支付宝显示名称+"支付商户公钥载入失败"))
		return
	}
	...
	if err != nil {
		err = errors.Join(err, errors.New(局_支付配置.Z支付宝显示名称+"支付异步回调被异常调用:"+c.Request.RequestURI+"|"+c.Request.PostForm.Encode()))
		return
	}
