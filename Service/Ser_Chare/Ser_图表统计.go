...
FROM (
    SELECT Ip, 
           CASE 
               WHEN Ip LIKE '%北京%' THEN '北京市'
               WHEN Ip LIKE '%上海%' THEN '上海市'
               WHEN Ip LIKE '%天津%' THEN '天津市'
               WHEN Ip LIKE '%重庆%' THEN '重庆市'
               WHEN Ip LIKE '%河北%' THEN '河南省'
               WHEN Ip LIKE '%山西%' THEN '山西省'
               WHEN Ip LIKE '%内蒙古%' THEN '内蒙古自治区'
               WHEN Ip LIKE '%辽宁%' THEN '辽宁省'
               WHEN Ip LIKE '%吉林%' THEN '吉林省'
               WHEN Ip LIKE '%黑龙江%' THEN '黑龙江省'
               WHEN Ip LIKE '%江苏%' THEN '江苏省'
               WHEN Ip LIKE '%浙江%' THEN '浙江省'
               WHEN Ip LIKE '%安徽%' THEN '安徽省'
               WHEN Ip LIKE '%福建%' THEN '福建省'
               WHEN Ip LIKE '%江西%' THEN '江西省'
               WHEN Ip LIKE '%山东%' THEN '山东省'
               WHEN Ip LIKE '%河南%' THEN '河南省'
               WHEN Ip LIKE '%湖北%' THEN '湖北省'
               WHEN Ip LIKE '%湖南%' THEN '湖南省'
               WHEN Ip LIKE '%广东%' THEN '广东省'
               WHEN Ip LIKE '%广西%' THEN '广西壮族自治区'
               WHEN Ip LIKE '%海南%' THEN '海南省'
               WHEN Ip LIKE '%四川%' THEN '四川省'
               WHEN Ip LIKE '%贵州%' THEN '贵州省'
               WHEN Ip LIKE '%云南%' THEN '云南省'
               WHEN Ip LIKE '%西藏%' THEN '西藏自治区'
               WHEN Ip LIKE '%陕西%' THEN '陕西省'
               WHEN Ip LIKE '%甘肃%' THEN '甘肃省'
               WHEN Ip LIKE '%青海%' THEN '青海省'
               WHEN Ip LIKE '%宁夏%' THEN '宁夏回族自治区'
               WHEN Ip LIKE '%新疆%' THEN '新疆维吾尔自治区'
               WHEN Ip LIKE '%台湾%' THEN '台湾省'
               WHEN Ip LIKE '%香港%' THEN '香港特别行政区'
               WHEN Ip LIKE '%澳门%' THEN '澳门特别行政区'
               ELSE '其他'
           END AS province
    FROM db_links_Token
) AS subquery
	...
	*/
	...
	global.GVA_DB.Model(DB.DB_LogRMBPayOrder{}).
		Select("SUM(case when ( Time between "+时间处理函数(-6)+" and "+时间处理函数(-5)+") then Rmb else null end) as  '1' ",
			...
			"SUM(case when ( Time between "+时间处理函数(-2)+" and "+时间处理函数(-0)+") then Rmb else null end) as  '5' ",
