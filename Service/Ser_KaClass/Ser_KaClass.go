...
func KaClassId是否存在(id int) bool {
	var Count int64
	result := global.GVA_DB.Model(DB.DB_KaClass{}).Select("1").Where("Id=?", id).First(&Count)
...
func KaName取map列表String(Appid int) map[string]string {
	...
	var 总数 int64
	_ = global.GVA_DB.Model(DB.DB_KaClass{}).Select("Id", "Name").Where("Appid=?", Appid).Count(&总数).Find(&DB_KaClass).Error
...
func KaClass取可购买卡类列表(Appid int) []DB.DB_KaClass {

	var DB_KaClass []DB.DB_KaClass
	_ = global.GVA_DB.Model(DB.DB_KaClass{}).Where("Appid=?", Appid).Where("Money>0").Find(&DB_KaClass).Error
	return DB_KaClass
}

func KaClass取详细信息(id int) (DB.DB_KaClass, error) {

	var KaClass详细信息 DB.DB_KaClass

	err := global.GVA_DB.Model(DB.DB_KaClass{}).Where("Id=?", id).First(&KaClass详细信息).Error

	return KaClass详细信息, err
}
func Id取详细信息_数组(id []int) ([]DB.DB_KaClass, error) {

	var KaClass详细信息 = make([]DB.DB_KaClass, 0, len(id))

	err := global.GVA_DB.Model(DB.DB_KaClass{}).Where("Id IN ?", id).Find(&KaClass详细信息).Error
...
func Id取Name(卡类id int) (Name string) {
	_ = global.GVA_DB.Model(DB.DB_KaClass{}).Select("Name").Where("Id=?", 卡类id).First(&Name).Error
	return Name
}
...
func Q取全部可制卡类树形框列表(上级代理ID int) []K可制卡类授权树形框结构 {
	...
	if 上级代理ID < 0 {
		...
		_ = global.GVA_DB.Model(DB.DB_KaClass{}).Where("AgentMoney>0").Find(&DB_KaClass).Error
	} else {
		var 上级代理可制卡类ID []int
		上级代理可制卡类ID, _ = Ser_Agent.Id取代理可制卡类和可用代理功能列表(上级代理ID)
		//只可以获取上级代理允许的ID
		_ = global.GVA_DB.Model(DB.DB_KaClass{}).Where("Id IN ?", 上级代理可制卡类ID).Where("AgentMoney>0").Find(&DB_KaClass).Error
	}
