...
func J极验_滑动验证码参数验证(验证码id, 验证码内容, 极验id, 极验Key string) int {
	...
	sign_token := hmac_encode(极验Key, lot_number)
	...
	resp, err := cli.PostForm("http://gcaptcha4.geetest.com/validate?captcha_id="+极验id, form_data)
