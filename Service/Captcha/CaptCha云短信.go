...
func Sms_当前选择发送短信验证码(模板变量 []string, 接收短信手机号 string) error {
	...
	case 5:
		return ALY阿里云_sms发送短信验证码(模板变量, 接收短信手机号)
	default:
...
func ALY阿里云_sms发送短信验证码(模板变量 []string, 接收短信手机号 string) error {
	局_阿里云 := setting.Q短信平台配置().Sms阿里云

	if 局_阿里云.AccessKeyId == "" {
		return 系统错误.New("Sms阿里云AccessKeyId配置无效")
	}
	if 局_阿里云.AccessKeySecret == "" {
		return 系统错误.New("Sms阿里云AccessKeySecret配置无效")
	}
	if 局_阿里云.D短信签名 == "" {
		return 系统错误.New("Sms阿里云短信签名配置无效")
	}
	if 局_阿里云.Z正文模板Code == "" {
		return 系统错误.New("Sms阿里云正文模板Code配置无效")
	}

	// 构造阿里云短信API请求参数
	params := map[string]string{
		"AccessKeyId":      局_阿里云.AccessKeyId,
		"Action":           "SendSms",
		"Format":           "JSON",
		"PhoneNumbers":     接收短信手机号,
		"RegionId":         "cn-hangzhou",
		"SignName":         局_阿里云.D短信签名,
		"SignatureMethod":  "HMAC-SHA1",
		"SignatureNonce":   utils.W文本_取随机字符串_数字(16),
	...
	for _, k := range keys {
		sortedParams = append(sortedParams, specialUrlEncode(k)+"="+specialUrlEncode(params[k]))
	}
	normalizedQueryString := strings.Join(sortedParams, "&")

	// 构造用于签名的字符串
	stringToSign := "GET&" + specialUrlEncode("/") + "&" + specialUrlEncode(normalizedQueryString)

	// HMAC-SHA1签名
	mac := hmac.New(sha1.New, []byte(局_阿里云.AccessKeySecret+"&"))
	mac.Write([]byte(stringToSign))
	signature := base64.StdEncoding.EncodeToString(mac.Sum(nil))

	// 构造最终请求URL
	requestUrl := "https://dysmsapi.aliyuncs.com/?" + normalizedQueryString + "&Signature=" + specialUrlEncode(signature)

	// 发送请求
	Http请求 := req.R()
	局_请求结果, err := Http请求.Get(requestUrl)
	if err != nil {
		return 系统错误.New("阿里云短信请求失败:" + err.Error())
	}
	...
	err = 局_请求结果.Unmarshal(&局_返回)
	if err != nil {
		return 系统错误.New("阿里云短信返回解析失败:" + err.Error())
	}
	...
	return 系统错误.New("阿里云短信发送失败:" + 局_返回.Code + " - " + 局_返回.Message)
}

// specialUrlEncode 阿里云专用URL编码（对 +、*、~ 等字符特殊处理）
