...
type DB_TaskPoolData struct {
	...
	Tid         int    `json:"Tid" gorm:"column:Tid;index:idx_tid_time_status;comment:对应的任务类型Id"`
	TimeStart   int    `json:"TimeStart" gorm:"column:TimeStart;index:idx_tid_time_status;index;comment:任务创建时间戳"` //时间增加索引,提高统计效率;复合索引(Tid,TimeStart,Status)覆盖索引,统计查询不回表
