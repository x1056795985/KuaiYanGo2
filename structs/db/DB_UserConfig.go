...
type DB_UserConfig struct {
	...
	Uid        int    `json:"Uid" gorm:"column:Uid;primarykey;comment:软件用户UiD; "`                                        //考虑了一下还是用Uid,不然无法找到用户名
	User       string `json:"User" gorm:"column:User;index;comment:用户登录名"`                                               //由于先通过appid判断uid是卡号还是用户名,在获取user,在批量读取列表时sql过于复杂,降低效率,所以直接多个字段保存user,
