...
type DB_LogLogin struct {
	...
	User      string `json:"User" gorm:"column:User;index;size:200;comment:用户名"`
	Time      int64  `json:"Time" gorm:"column:Time;comment:时间"`
	Ip        string `json:"Ip" gorm:"column:Ip;size:200;comment:登录ip地址"`
	LoginType int    `json:"LoginType" gorm:"column:LoginType;comment:登录用户类型"` //0 普通用户  1 2 3 级代理  4  管理员
