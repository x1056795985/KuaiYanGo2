...
type DB_User struct {
	...
	PassWord            string  `json:"PassWord" gorm:"column:PassWord;size:200;comment:用户登录密码"`              // 用户登录密码
	...
	SuperPassWord       string  `json:"SuperPassWord" gorm:"column:SuperPassWord;size:200;comment:超级密码或密保答案"` // 超级密码或密保答案
	...
	LoginIp             string  `json:"LoginIp" gorm:"column:LoginIp;size:200;comment:登录ip"`
	LoginTime           int64   `json:"LoginTime" gorm:"column:LoginTime;comment:登录时间"`
	RegisterIp          string  `json:"RegisterIp" gorm:"column:RegisterIp;size:200;comment:注册ip"`
