...
func (a *Api) GetInfo(c *gin.Context) {
	...
	var DB_LogVipNumber DB.DB_LogVipNumber

	err = global.GVA_DB.Model(DB.DB_LogVipNumber{}).Where("Id= ?", 请求.Id).First(&DB_LogVipNumber).Error
	// 没查到数据
	if err != nil {
		response.FailWithMessage("获取失败,可能不存在", c)
		return
	}
	response.OkWithDetailed(DB_LogVipNumber, "获取成功", c)
	return
}

type 结构请求_GetDB_LogVipNumberList struct {
	...
	LogType      int      `json:"LogType"`      // 1 积分 2 点数
	AppId        int      `json:"AppId"`        // 指定Appid
}

// GetDB_LogVipNumberList

func (a *Api) GetLogList(c *gin.Context) {
	var 请求 结构请求_GetDB_LogVipNumberList
	...
	局_DB := global.GVA_DB.Model(DB.DB_LogVipNumber{})
	if 请求.Order == 1 {
		局_DB.Order("Id ASC")
	} else {
		局_DB.Order("Id DESC")
	...
	err = 局_DB.Limit(请求.Size).Offset((请求.Page - 1) * 请求.Size).Find(&DB_LogVipNumber).Error
	...
	response.OkWithDetailed(结构响应_GetDB_LogVipNumberList{DB_LogVipNumber, 总数}, "获取成功", c)
	return
}

type 结构响应_GetDB_LogVipNumberList struct {
...
func (a *Api) Delete(c *gin.Context) {
	...
	var 影响行数 int64
	var db = global.GVA_DB.Model(DB.DB_LogVipNumber{})
	...
	case 1:
		...
		影响行数 = db.Where("Id IN ? ", 请求.Id).Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 2:
		影响行数 = db.Where("User = ? ", 请求.Keywords).Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 3: //清空
		影响行数 = db.Where("1=1").Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 4: //删7天前
		影响行数 = db.Where("Time <  ?", time.Now().Unix()-604800).Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 5: //删除30天前
		影响行数 = db.Where("Time <  ?", time.Now().Unix()-2592000).Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 6: //删除90天前
		影响行数 = db.Where("Time <  ?", time.Now().Unix()-7776000).Delete(DB.DB_LogVipNumber{}).RowsAffected
	case 7: //删除关键字
