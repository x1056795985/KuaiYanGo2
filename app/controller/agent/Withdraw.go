...
func (C *Withdraw) Create(c *gin.Context) {
	...
	info, err := rmbWithdrawLogic.T提现_创建(global.GVA_DB, c.GetInt("Uid"), c.GetString("User"), c.ClientIP(), req)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
...
func (C *Withdraw) Cancel(c *gin.Context) {
	...
	if err := rmbWithdrawLogic.T提现_取消(global.GVA_DB, req.Id, c.GetInt("Uid"), c.GetString("User"), c.ClientIP()); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
