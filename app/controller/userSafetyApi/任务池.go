...
func UserApi_任务池_任务创建(c *gin.Context) {
	...
	if 局_任务类型.HookSubmitDataStart != "" {
		局_任务数据, _, err = jsEngine.J脚本引擎_处理任务池Hook(c, &局_ctx.AppInfo, &局_ctx.Z在线信息, 局_任务类型.HookSubmitDataStart, 局_任务数据, 0)
		if err != nil {
	...
	if 局_任务类型.HookSubmitDataEnd != "" {
		局_任务数据, _, err = jsEngine.J脚本引擎_处理任务池Hook(c, &局_ctx.AppInfo, &局_ctx.Z在线信息, 局_任务类型.HookSubmitDataEnd, 局_任务数据, 1)
		if err != nil {
			response.FailMsg(c, constant.Status_操作失败, err.Error())
			return
		}
...
func UserApi_任务池_任务处理返回(c *gin.Context) {
	...
	if 局_任务类型.HookReturnDataStart != "" {
		局_任务数据, 局_任务状态, err = jsEngine.J脚本引擎_处理任务池Hook(c, &局_ctx.AppInfo, &局_ctx.Z在线信息, 局_任务类型.HookReturnDataStart, 局_任务数据, 局_任务状态)
		if err != nil {
			response.FailMsg(c, constant.Status_操作失败, err.Error())
			return
		}
	...
	if 局_任务类型.HookReturnDataEnd != "" {
		局_任务数据, 局_任务状态, err = jsEngine.J脚本引擎_处理任务池Hook(c, &局_ctx.AppInfo, &局_ctx.Z在线信息, 局_任务类型.HookReturnDataEnd, 局_任务数据, 局_任务状态)
		if err != nil {
			response.FailMsg(c, constant.Status_操作失败, err.Error())
			return
		}
