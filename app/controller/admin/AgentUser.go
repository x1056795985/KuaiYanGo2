...
func (C *AgentUser) GetKaSalesStatistics(c *gin.Context) {
	...
	info.appInfo = service.NewAppInfo(c, &tx).App取App详情(请求.AppId)

	if 请求.AgentName != "" {
		局_代理info, ok := service.NewUser(c, &tx).User取详情(请求.AgentName)
		if !ok {
	...
	for _, item制卡人User := range 局_制卡人列表 {
		...
		for 局_卡类id, 局_卡类id数量 := range 局_卡类map {
			info.卡类id名称map[局_卡类id] = service.NewKaClass(c, &tx).Id取Name(局_卡类id)
