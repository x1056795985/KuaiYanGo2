...
func (j L_taskPool) Task数据创建加入队列(c *gin.Context, 任务类型Id int, 生产提交数据 string, SubmitAppId, SubmitUid int) (string, error) {
	...
	err := 局_db.Model(dbm.DB_TaskPoolData{}).Create(&局_数据).Error
	if err != nil {
		return "", err
	}

	局_队列 := dbm.TaskPool_队列{
		Uuid: 局_数据.Uuid,
		Tid:  局_数据.Tid,
	}
	err = 局_db.Model(dbm.TaskPool_队列{}).Create(&局_队列).Error
	if err != nil {
		//如果失败任务删除丢弃,除非雪崩,不然概率不大,大量出就人工介入
		_ = 局_db.Model(dbm.DB_TaskPoolData{}).Delete(&局_数据)
		return "", err
	}
...
func (j L_taskPool) Task数据读取_数组(Uuid []string) []dbm.TaskPool_数据_精简 {
	var TaskPool_数据 []dbm.TaskPool_数据_精简
	if len(Uuid) == 0 {
		return TaskPool_数据
	}
	db := *global.GVA_DB
	_ = db.Model(dbm.DB_TaskPoolData{}).Where("Uuid in ?", Uuid).Find(&TaskPool_数据).Error
	return TaskPool_数据
}
...
func (j L_taskPool) Task数据读取Tid(db *gorm.DB, Uuid string) int {
	var Tid int
	_ = db.Model(dbm.DB_TaskPoolData{}).Select("Tid").Where("Uuid = ?", Uuid).First(&Tid).Error
	return Tid
}
