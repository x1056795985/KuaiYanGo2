...
func (j *agent) S删除代理(c *gin.Context, UID []int) error {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		// 删除代理关系
		影响行数 = tx.Model(dbm.Db_Agent_Level{}).Where("Uid IN ? ", UID).Delete(dbm.Db_Agent_Level{}).RowsAffected
		if 影响行数 == 0 {
		...
		if err := tx.Model(&dbm.DB_UserConfig{}).Where("Uid IN ? ", UID).Delete(&dbm.DB_UserConfig{}).Error; err != nil {
...
func (j *agent) ID取分成百分比(c *gin.Context, UID int) (分成百分比 int) {
	db := *global.GVA_DB
	if UID > 0 {
		db.Model(dbm.DB_User{}).Select("AgentDiscount").Where("Id=?", UID).First(&分成百分比)
	} else if UID < 0 {
		db.Model(dbm.DB_Admin{}).Select("AgentDiscount").Where("Id=?", -UID).First(&分成百分比)
	}
	return
}
...
func (j *agent) Id取代理可制卡类和可用代理功能列表(c *gin.Context, 代理ID int) ([]int, []int) {
	...
	var 功能权限 = []int{}
	db := *global.GVA_DB
	db.Model(dbm.Db_Agent_卡类授权{}).Select("Kid").Where("Uid=?", 代理ID).Find(&临时)
...
func (j *agent) D代理授权卡类Id删除(c *gin.Context, kid int) (int64, error) {
	db := *global.GVA_DB
	ret := db.Model(dbm.Db_Agent_卡类授权{}).Select("Kid").Where("kid=?", kid).Delete("")
...
func (j *agent) Z置Id代理可制卡类或功能授权列表(c *gin.Context, 代理ID int, 授权卡类ID []int) error {
	...
	if len(新增卡类ID) > 0 {
		var 新授权记录 []dbm.Db_Agent_卡类授权
		for _, 卡类ID := range 新增卡类ID {
			新授权记录 = append(新授权记录, dbm.Db_Agent_卡类授权{
				Uid: 代理ID,
				KId: 卡类ID,
			})
		}
...
func (j *agent) Q取下级代理数组(c *gin.Context, 上级ID []int) []int {
	var 下级代理 = []int{}
	db := *global.GVA_DB
	db.Model(dbm.Db_Agent_Level{}).Select("Uid").Where("UPAgentId IN ?", 上级ID).Where("Level=1").Find(&下级代理)
	return 下级代理
}
...
func (j *agent) Q取下级代理数组_user(c *gin.Context, 上级ID []int) []string {
	...
	db := *global.GVA_DB
	db.Model(dbm.DB_User{}).Select("User").Where("Id IN ?", 局_数组_uid).Find(&局_制卡人数组)
	return 局_制卡人数组
...
func (j *agent) Q取下级代理数组含子级(c *gin.Context, 上级ID []int) []int {
	var 下级代理 = []int{}
	db := *global.GVA_DB
	db.Model(dbm.Db_Agent_Level{}).Select("Uid").Where("UPAgentId IN ?", 上级ID).Find(&下级代理)
	return 下级代理
}
...
func (j *agent) Q取上级代理的子级代理级别(c *gin.Context, 上级ID, 子级代理ID int) int {
	var 局_临时整数 = 0
	db := *global.GVA_DB
	db.Model(dbm.Db_Agent_Level{}).Select("Level").Where("Uid = ?", 子级代理ID).Where("UPAgentId = ?", 上级ID).Take(&局_临时整数)
	return 局_临时整数
}
...
func (j *agent) S是否都为子级代理(c *gin.Context, 上级ID int, 子级代理ID []int) bool {
	var 局_临时整数 []int
	db := *global.GVA_DB
	db.Model(dbm.Db_Agent_Level{}).Select("Level").Where("Uid IN ?", 子级代理ID).Where("UPAgentId = ?", 上级ID).Find(&局_临时整数)
...
func (j *agent) Q取Id数组中代理数量(c *gin.Context, 代理ID []int) int {
	var 局_临时整数 []int
	db := *global.GVA_DB
	db.Model(dbm.DB_User{}).Select("Id").Where("id IN ?", 代理ID).Where("UPAgentId != 0").Find(&局_临时整数)
	//查询出来的数量=0 则说明 没有代理
	return len(局_临时整数)
}
...
func (j *agent) Q取全部代理功能ID_MAP(c *gin.Context) map[int]string {
	...
	局_map[dbm.D代理功能_卡号冻结] = "卡号冻结"
	局_map[dbm.D代理功能_卡号解冻] = "卡号解冻"
	局_map[dbm.D代理功能_更换卡号] = "更换卡号"
	//局_map[DB.D代理功能_删除卡号] = "删除卡号"
	局_map[dbm.D代理功能_余额充值] = "余额充值"
	局_map[dbm.D代理功能_发展下级代理] = "发展下级代理"
	局_map[dbm.D代理功能_卡号追回] = "卡号追回"
	局_map[dbm.D代理功能_修改用户绑定] = "修改用户绑定"
	局_map[dbm.D代理功能_转账] = "转账"
	局_map[dbm.D代理功能_代收款] = "代收款"
	局_map[dbm.D代理功能_查看归属软件用户] = "查看归属软件用户"
	局_map[dbm.D代理功能_冻结软件用户] = "冻结软件用户"
	局_map[dbm.D代理功能_解冻软件用户] = "解冻软件用户"
	局_map[dbm.D代理功能_修改用户密码] = "修改用户密码"
	局_map[dbm.D代理功能_卡类调价] = "卡类调价"
	局_map[dbm.D代理功能_制卡] = "制卡"
	return 局_map
}
...
func (j *agent) Z执行调价信息分成(c *gin.Context, 调价详情 []dbm.DB_KaClassUpPrice, 购买数量 int64, 日志前缀 string) (err error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		日志记录集 := make([]dbm.DB_LogMoney, 0, len(调价详情))
		...
		// 批量插入日志
		if err = tx.Model(dbm.DB_LogMoney{}).Create(&日志记录集).Error; err != nil {
...
func (j *agent) Z执行百分比代理分成(c *gin.Context, 分成结构 []m.D代理分成结构, 总额度 float64, 日志前缀 string, isShowLog bool) (err error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		日志记录集 := make([]dbm.DB_LogMoney, 0, len(分成结构))
		...
		for _, d := range 分成结构 {
			...
			var 局_userInfo dbm.DB_User
			err = tx.Model(dbm.DB_User{}).Where("Id = ?", d.Uid).Find(&局_userInfo).Error
			if err != nil {
