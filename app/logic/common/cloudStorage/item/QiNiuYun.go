...
func (j *Q七牛云) Q取文件信息(c *gin.Context, 文件路径 string) (common.W文件对象详情, error) {
	if j == nil || j.bucket == nil {
		return common.W文件对象详情{}, errors.New("七牛云对象存储未初始化")
	}
	key := j.配置.RootPath + 文件路径
	对象详情, err := j.bucket.Object(key).Stat().Call(Q七牛云_取上下文(c))
	if err != nil {
		return common.W文件对象详情{}, err
	}
	if 对象详情 == nil {
		return common.W文件对象详情{}, errors.New("七牛云返回的文件信息为空")
	}
	return common.W文件对象详情{
		Name:   W文件_取文件名(对象详情.Name),
		Path:   对象详情.Name[len(j.配置.RootPath):],
		MD5:    Z字节集_字节集到十六进制(对象详情.MD5[:]),
		Size:   对象详情.Size,
		Type:   2,
		UpTime: 对象详情.UploadedAt.Unix(),
	}, nil
}

func Q七牛云_取上下文(c *gin.Context) context.Context {
