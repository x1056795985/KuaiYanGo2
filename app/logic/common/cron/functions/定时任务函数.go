...
// 每日00:00分执行
func D定时任务_统计初始化日活月活(c *gin.Context) {
	if global.GVA_DB == nil {
		return
	}
	tx := *global.GVA_DB
...
`).Scan(&AppIds).Error
	for _, v := range AppIds {
		...
		if 日活记录.Id == 0 {
			...
			//查appid在线数
			_ = tx.Raw(`SELECT COUNT(*)   FROM db_links_Token  WHERE Status = 1 AND LoginAppid=?`, v).
...
func D定时任务_执行公共函数(时间戳 int64, R任务数据 db.DB_Cron) (string, error) {
	...
	vm := jsEngine.J脚本引擎_初始化用户(&gin.Context{}, &dbm.DB_AppInfo{}, &dbm.DB_LinksToken{}, &局_js数据)
