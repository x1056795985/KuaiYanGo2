...
func (j *ka) Q取全部可制卡类树形框列表(c *gin.Context, 上级代理ID int) []K可制卡类授权树形框结构 {
	...
	db := *global.GVA_DB
	_ = db.Model(dbm.DB_AppInfo{}).Select("AppId", "AppName").Find(&DB_AppInfo).Error

	var DB_KaClass []dbm.DB_KaClass
	if 上级代理ID < 0 {
		//如果小于0说明是开发者,或管理员,可以获取全部卡类   代理价格-1为禁止代理购买的卡号
		_ = db.Model(dbm.DB_KaClass{}).Where("AgentMoney>0").Find(&DB_KaClass).Error
	} else {
		var 上级代理可制卡类ID []int
		上级代理可制卡类ID, _ = agent.L_agent.Id取代理可制卡类和可用代理功能列表(c, 上级代理ID)
		//只可以获取上级代理允许的ID
		_ = db.Model(dbm.DB_KaClass{}).Where("Id IN ?", 上级代理可制卡类ID).Where("AgentMoney>0").Find(&DB_KaClass).Error
	}
