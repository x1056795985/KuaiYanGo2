...
func (r *AllRouter) InitAgentRouter(router *gin.RouterGroup) {
	...
	agentUserController := controller.NewAgentUserController()
	{
		...
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_发展下级代理})).POST("Agent/NewAgentUser", agentUserController.New代理信息)
		...
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_转账})).POST("Agent/SendRmbTOAgent", agentUserController.SendRmbTOAgent)
	...
	kaController := controller.NewAgentKaController()
	{
		...
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_制卡})).POST("Ka/New", kaController.New)
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_制卡})).POST("Ka/InventoryNewKa", kaController.K库存制卡)
		agentRouter.POST("Ka/SetStatus", kaController.Set修改状态)
		agentRouter.POST("Ka/SetAgentNote", kaController.Set修改代理备注)
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_卡号追回})).POST("Ka/Recover", kaController.Z追回卡号)
		agentRouter.Group("", mid2.Is代理鉴权([]int{dbm.D代理功能_更换卡号})).POST("Ka/ReplaceKaName", kaController.G更换卡号)
