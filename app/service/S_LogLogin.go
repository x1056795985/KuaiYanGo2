...
func (s *S_LogLogin) Info(tx *gorm.DB, Id int) (dbm.DB_LogLogin, error) {
	var value dbm.DB_LogLogin
	err := tx.Model(dbm.DB_LogLogin{}).Where("Id = ?", Id).First(&value).Error
	return value, err
}

func (s *S_LogLogin) GetList(tx *gorm.DB, 请求 request.List, Appid int, RegisterTime []string) (int64, []dbm.DB_LogLogin, error) {
	局_DB := tx.Model(dbm.DB_LogLogin{})
	if 请求.Order == 1 {
		局_DB.Order("Id ASC")
	} else {
		局_DB.Order("Id DESC")
...
func (s *S_LogLogin) BatchDelete(tx *gorm.DB, Id []int, Type int, Keywords string) (int64, error) {
	var 影响行数 int64
	var d = tx.Model(dbm.DB_LogLogin{})
	...
	case 1:
		...
		影响行数 = d.Where("Id IN ?", Id).Delete(dbm.DB_LogLogin{}).RowsAffected
	case 2:
		影响行数 = d.Where("User = ?", Keywords).Delete(dbm.DB_LogLogin{}).RowsAffected
	case 3:
		影响行数 = d.Where("1=1").Delete(dbm.DB_LogLogin{}).RowsAffected
	case 4:
		影响行数 = d.Where("Time < ?", time.Now().Unix()-604800).Delete(dbm.DB_LogLogin{}).RowsAffected
	case 5:
		影响行数 = d.Where("Time < ?", time.Now().Unix()-2592000).Delete(dbm.DB_LogLogin{}).RowsAffected
	case 6:
		影响行数 = d.Where("Time < ?", time.Now().Unix()-7776000).Delete(dbm.DB_LogLogin{}).RowsAffected
	case 7:
		if len(Keywords) == 0 {
			return 0, errors.New("关键字不能为空")
		}
		影响行数 = d.Where("LOCATE(?, Note)>0", Keywords).Delete(dbm.DB_LogLogin{}).RowsAffected
	}
